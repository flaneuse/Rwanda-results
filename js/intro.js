...
var dims = {
  ...
  map: { 
    w: width - margins.map.right - margins.map.left,
...
function show4() {
  ...
  .styleTween("transform", function() {
    return d3.interpolate("translate(" + lakeVicX + "," + lakeVicY + ") scale(" + lakeVicScale + ")", "translate(0%, 0%) scale(1)");
  });
...
function show5(){
  ...
  .each("end", function() {
    ...
    .duration(2000)
    .styleTween("transform", function() {
      return d3.interpolate("translate(0%, 0%) scale(1)", "translate(" + lakeVicX + "," + lakeVicY + ") scale(" + lakeVicScale + ")");
    })
    .each("end", function() {
...
function show6() {
  ...
  .duration(0)
  .styleTween("transform", function() {
    return d3.interpolate("translate(" + RWA_xformX + "," + RWA_xformY + ") scale(" + RWA_xformScale + ")", "translate(0%, 0%) scale(1)");
  })
...
function show7() {
  ...
  fullG.select("#afr3").selectAll("img")
    ...
    .duration(1000)
    .styleTween("transform", function() {
      return d3.interpolate("translate(0%, 0%) scale(1)", "translate(" + RWA_xformX + "," + RWA_xformY + ") scale(" + RWA_xformScale + ")");
    })
    .each("end", function() {
      // overlay the higher resolution version of the image
      fullG.select("#afr3")
...
function animatePath(id, tDuration){
  ...
  path
    ...
    .style("opacity", 1)
    .transition()
    .duration(tDuration).ease("linear")
    .attr("stroke-dashoffset", 0);
}
